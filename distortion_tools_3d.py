...
def successiveDistances3D(array):
    """Calculates euclidian distance btw sets of points in an array"""
    nb_of_points = array.shape[0]
    diffs = np.zeros(nb_of_points - 1, dtype=float)
    for i in range(0, nb_of_points - 1):
        diffs[i] = (array[i][0] - array[i + 1][0]) ** 2 + (array[i][1] - array[i + 1][1]) ** 2 + (array[i][2] - array[i + 1][2]) ** 2
...
def applyTranslation(array, translation):
    ...
    affine_tform = tf.AffineTransform(
        matrix=None, scale=None, rotation=None, shear=None, translation=translation
    )
    arrayTranslated = tf.warp(
        array,
        affine_tform,
        cval=0,  # new pixels are black pixel
        order=0,  # k-neighbour
