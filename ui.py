...
class App(tk.Tk):
    def __init__(self, screenName=None, baseName=None):
        ...
        filemenu.add_command(
            label="Export 3D", command=lambda: self.apply_correction_to_h5("TPS")
        )
        filemenu.add_command(
            label="Open 2D", command=lambda: self.select_data_popup("2D")
        )
        filemenu.add_command(
            label="Export 2D", command=lambda: self.apply_correction_to_tif("TPS")
        )
        ...
        self.ex_ctr_pt_ims = ttk.Button(
            self.top,
            text="Export control point images",
            command=self.export_CP_imgs,
            state="disabled",
        )
        ...
        self.clahe_b = ttk.Button(
            self.bot_right, text="Apply CLAHE", command=self.clahe, state="disabled"
        )
    ...
    def apply(self, algo="TPS"):
        ...
        result = tk.messagebox.askyesnocancel(
            "Crop?",
            "Would you like to crop the corrected output to match the distorted grid?",
        )
        if result is None:
            return
        im0, im1 = self._run_in_background(
            "Applying correction...", self._apply, algo, result
        )
    ...
    def _apply(self, algo, crop_to_distorted_grid):
        ...
        if crop_to_distorted_grid:
            ...
            print(
                "Aligned/Control data cropped from {} to {} (to match distorted grid size)".format(
                    im1.shape, im1[rslc, cslc].shape
                )
            )
            im0 = im0[rslc, cslc]
            im1 = im1[rslc, cslc]
    ...
    def apply_3D(self, algo="LR"):
        """Applies the correction algorithm and calls the interactive view"""
        crop_to_dist_grid = tk.messagebox.askyesnocancel(
            "Crop?",
            "Would you like to crop the output to match the distorted grid (usually yes)?",
        )
        if crop_to_dist_grid is None:
            return
        bse_stack, ebsd_cStack = self._run_in_background(
            "Applying correction...", self._apply_3D, algo, crop_to_dist_grid
        )
        # View
        print("Creating interactive view")
    ...
    def _apply_3D(self, algo, crop_to_ebsd_grid):
        ...
        ebsd_stack = np.around(
            255
            * ((ebsd_stack - ebsd_stack.min()) / (ebsd_stack.max() - ebsd_stack.min())),
            0,
        ).astype(np.uint8)
        ...
        referencePoints = np.array(
            self.points["bse"][list(self.points["bse"].keys())[0]]
        )
        distortedPoints = np.array(
            self.points["ebsd"][list(self.points["ebsd"].keys())[0]]
        )
        ...
        if crop_to_ebsd_grid:
            ...
            rslc, cslc = self._get_cropping_slice(
                (rc, cc), ebsd_stack.shape[1:], ebsd_cStack.shape[1:]
            )
            print(
                "Aligned/Control data cropped from {} to {} (to match EBSD grid)".format(
                    ebsd_cStack.shape[1:], ebsd_cStack[:, rslc, cslc].shape
                )
            )
    ...
    def apply_correction_to_h5(self, algo):
        ...
        EBSD_DIR_CORRECTED = filedialog.asksaveasfilename(
            initialdir=os.path.basename(self.ebsd_path),
            title="Save the corrected data in a new Dream3d file",
            filetypes=[("Dream3D HDF5 File", "*.dream3d"), ("All files", "*.*")],
        ...
        for key in keys:
            ...
            for i in range(ebsd_stack.shape[-1]):
                ...
                rslc, cslc = self._get_cropping_slice(
                    (rc, cc), stack.shape[1:], c_stack.shape[1:]
                )
                c_stack = c_stack[:, rslc, cslc]
                if dtype == np.uint8:
                    c_stack = np.around(255 * c_stack / c_stack.max(), 0).astype(
                        np.uint8
                    )
                elif dtype == np.uint16:
                    c_stack = np.around(65535 * c_stack / c_stack.max(), 0).astype(
                        np.uint16
                    )
                elif dtype == np.uint32:
                    c_stack = np.around(4294967295 * c_stack / c_stack.max(), 0).astype(
                        np.uint32
                    )
                elif dtype == np.uint64:
                    c_stack = np.around(
                        18446744073709551615 * c_stack / c_stack.max(), 0
                    ).astype(np.uint64)
                else:
    ...
    def apply_correction_to_tif(self, algo):
        ...
        if SAVE_PATH_EBSD != "":
            ...
            if extension != ".ang":
                # Get the images
                ebsd_im = np.squeeze(
                    self.ebsd_data[self.ebsd_mode.get()][int(self.slice_num.get())]
                )
                ...
                if result:
                    ...
                    # Now crop the corrected image
                    rslc, cslc = self._get_cropping_slice(
                        (rc, cc), ebsd_im.shape, aligned.shape
                    )
            ...
            SAVE_PATH_BSE = filedialog.asksaveasfilename(
                initialdir=os.path.basename(self.ebsd_path),
                title="Save control image",
                filetypes=[("TIF", "*.tif"), ("TIFF", "*.tiff"), ("All files", "*.*")],
    ...
    def export_CP_imgs(self):
        ...
        self._save_CP_img(
            f"{i}_ebsd", self.ebsd_data[ebsd_modality][i], pts, "gray", "#c2344e"
        )
